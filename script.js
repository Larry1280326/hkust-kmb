...
async function fetchStopEta(stopId, route, config) {
  try {
    ...
    if (config.minibus) {
      let response;
      try {
        // First attempt: Larry128-host-test API
        const firstResponse = await fetch(
          `https://Larry128-host-test.hf.space/minibus?stop_id=${stopId}`
        );
        ...
        if (firstResponse.ok) {
          ...
          throw new Error(
            `First URL failed with HTTP status ${firstResponse.status}`
          );
        }
      } catch (firstError) {
        try {
          // Second attempt: Official ETAGMB API
          ...
          if (secondResponse.ok) {
            ...
            throw new Error(
              `Fallback URL failed with HTTP status ${secondResponse.status}`
            );
          }
        } catch (secondError) {
          // Both APIs failed
          throw new Error(
            `Both URL attempts failed. First error: ${firstError.message}, Second error: ${secondError.message}`
          );
        }
      }

      // Parse response (guaranteed to be successful at this point)
...
async function fetchAllEtas() {
  ...
  document.body.className = `route-${currentRoute
    .toLowerCase()
    .replace("m", "")}`;
  ...
  try {
    ...
    stopsContainer.innerHTML = config.stops
      .map((stop, index) => createStopCard(stop, allEtas[index], config.route))
      .join("");
...
function initApp() {
  ...
  document
    .getElementById("toPoLamTab")
    .addEventListener("click", () => switchDirection("polam"));
  document
    .getElementById("toDiamondHillTab")
    .addEventListener("click", () => switchDirection("diamondhill"));
  document
    .getElementById("toClearWaterBayTab")
    .addEventListener("click", () => switchDirection("clearwaterbay"));
  document
    .getElementById("toDiamondHill91Tab")
    .addEventListener("click", () => switchDirection("diamondhill91"));
  document
    .getElementById("toHkustNorthTab")
    .addEventListener("click", () => switchDirection("hkustnorth"));
  document
    .getElementById("toPingShekTab")
    .addEventListener("click", () => switchDirection("pingshek"));
  document
    .getElementById("toHangHauTab")
    .addEventListener("click", () => switchDirection("tohanghau"));
  document
    .getElementById("toChoiHungTab")
    .addEventListener("click", () => switchDirection("tochoihung"));
